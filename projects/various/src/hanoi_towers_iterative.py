...
class HanoiTowers:
    ...
    def move_disc_between_towers(self, tower1, tower2, to_log=False):
        ...
        if tower1_top_disc == no_disc_value:
            ...
            to_log and log.info(f"Move disc {tower1_top_disc} from {tower1.name} to {tower2.name}")
    ...
    def move_discs_to_end_tower(self, to_log=False):
        ...
        for move in range(1, total_moves + 1):
            if move % 3 == 1:
                self.move_disc_between_towers(start, end, to_log=to_log)
            elif move % 3 == 2:
                self.move_disc_between_towers(start, tmp, to_log=to_log)
            elif move % 3 == 0:
                self.move_disc_between_towers(tmp, end, to_log=to_log)
...
def main():
    ...
    hanoi_towers.move_discs_to_end_tower(to_log=True)
